var common = (function(){
  ...
  document.getElementById('subscriptionKeyInput').style.visibility = 'hidden';
  document.getElementById('subscriptionRegionSelect').style.visibility = 'hidden';
  var subscriptionChange = function(){
    // Build parameter string.
    var paramString = "?subscriptionKey=" + 
      encodeURIComponent(document.getElementById("subscriptionKeyInput").value) + "&subscriptionRegion=" + 
      encodeURIComponent(document.getElementById("subscriptionRegionSelect").value);
    
    var urlString = uriBaseAnalyze;
    
    // Check for existing URL parameters.
    var pos = urlString.indexOf("?");
    if (pos === -1){
        // No URL parameters are attached to the navigation URLs.
        uriBaseAnalyze = urlString + paramString;
    }
    else{
        // Trim off the existing URL parameters before adding the current parameters.
        uriBaseAnalyze = urlString.substring(0, pos) + paramString
    }
  };
